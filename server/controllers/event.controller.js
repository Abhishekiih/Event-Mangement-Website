...
module.exports = {
    ...
    post: {
        create: (req, res, next) => {
            const {description, location, name, date, imageURL, googleform} = req.body;
            const {_id} = req.user;

            models.Event.create({description, location, name, date, imageURL, googleform, admin: _id})
                .then((createdEvent) => {
                    return Promise.all([
    ...
    put: {
        edit: (req, res, next) => {
            const id = req.params.id;
            const {name, description, imageURL, date, location, googleform} = req.body;
            models.Event.findByIdAndUpdate(id, {name, description, imageURL, date, location, googleform})
